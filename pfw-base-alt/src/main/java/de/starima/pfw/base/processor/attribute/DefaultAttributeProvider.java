...
public class DefaultAttributeProvider extends AbstractProcessor implements IAttributeProviderProcessor<RcnAttribute> {
    ...
    private List<RcnAttribute> mergedAttributes;

    @Override
    public List<RcnAttribute> getAttributes() {
        if (mergedAttributes == null) {
            List<RcnAttribute> parentAttributes = new ArrayList<>();
            ...
            this.mergedAttributes = MapUtils.mergeLists(parentAttributes, this.attributes != null ? this.attributes : new ArrayList<RcnAttribute>(),
    ...
    @Override
    public RcnAttribute getAttribute(String name) {
    ...
    public List<RcnAttribute> getAttributes(List<String> names) {
        if (names != null) {
            ArrayList<RcnAttribute> result = new ArrayList<>();
    ...
    public List<String> getAttributeReferences() {
        List<RcnAttribute> collectedAttributes = getAttributes();
        if (collectedAttributes != null) {
            try {
                return collectedAttributes.stream().map(RcnAttribute::getName).collect(Collectors.toList());
            } catch (Exception e) {
