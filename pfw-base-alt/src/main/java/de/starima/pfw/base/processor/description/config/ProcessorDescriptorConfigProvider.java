...
@Slf4j
@Processor(description = "Erzeugt eine IDescriptorConfig fÃ¼r Prozessoren, indem es Annotationen aus der Klassenhierarchie mergt.")
public class ProcessorDescriptorConfigProvider extends AbstractProcessor implements IDescriptorConfigProvider {
    public boolean isResponsibleFor(IDescriptorConfigCreationContext context) {
        Type type = getTypeFromContext(context);
        return (type instanceof Class) && ProcessorUtils.isConsideredProcessor((Class<?>) type);
    }
    ...
    public IDescriptorConfig provide(IDescriptorConfigCreationContext context) {
        ...
        List<Processor> processorAnnotations = ProcessorUtils.findAnnotationsInHierarchy(processorClass, Processor.class);
        ...
        ProcessorConfig processorConfig = mergeProcessorAnnotations(processorClass, context.getProcessorParameter());
    ...
    private ProcessorConfig mergeProcessorAnnotations(Class<?> processorClass, ProcessorParameter parameterAnnotation) {
        ...
        for (Processor p : processorAnnotations) {
            ...
            if (p.defaultBeanParameterMapFileName() != null && !p.defaultBeanParameterMapFileName().isEmpty()) {
                processorConfig.setDefaultBeanParameterMapFileName(p.defaultBeanParameterMapFileName());
            }
