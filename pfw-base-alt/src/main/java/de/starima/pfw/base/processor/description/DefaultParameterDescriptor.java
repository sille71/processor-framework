...
public class DefaultParameterDescriptor extends DescriptorProcessor implements IParameterDescriptor {
    ...
    public List<IValueDescriptor> getPossibleValueDescriptors(IProposalContext context) {
        ...
        // 1. Hole den "Katalog" aller mÃ¶glichen Kandidaten vom Resolver
        List<Map<String, Map<String, Object>>> allCandidateMaps = descriptorResolver.findAllCandidateDescriptorMaps();
        List<IValueDescriptor> possibleValues = new ArrayList<>();

        //TODO: ist zu Ã¼berarbeiten
        // 2. Entscheide, welche Filterstrategie angewendet wird
        // PrÃ¼fe, ob explizite Filter (requiredCategories/Tags) gesetzt sind
        boolean useExplicitFilter = true;
        //boolean useExplicitFilter = (getRequiredCategories() != null && getRequiredCategories().length > 0) ||
          //                          (getRequiredTags() != null && getRequiredTags().length > 0) ||
            //                        (getRequiredSubCategories() != null && getRequiredSubCategories().length > 0);

        for (Map<String, Map<String, Object>> candidateMap : allCandidateMaps) {
            // Erzeuge temporÃ¤r den Deskriptor des Kandidaten, um ihn zu prÃ¼fen
            // Hier wird der createProcessor des Frameworks genutzt, um den Deskriptor zu instanziieren
            IValueDescriptor candidateDescriptor = createDescriptorFromMap(candidateMap);
            if (candidateDescriptor == null) continue;
            // FÃ¼r das Matching benÃ¶tigen wir die Metadaten des Kandidaten.
            // Annahme: IValueDescriptor kann zu IStructureValueDescriptor gecastet werden,
            // wenn es sich um einen @Processor oder @ValueObject handelt, die Kategorien/Tags haben.
            // Skalare Typen haben diese nicht und werden anders behandelt.
            if (candidateDescriptor instanceof IStructureValueDescriptor) {
                IStructureValueDescriptor structuredCandidate = (IStructureValueDescriptor) candidateDescriptor;
                if (useExplicitFilter) {
                    if (matchesExplicitly(structuredCandidate)) {
                        possibleValues.add(candidateDescriptor);
                    }
                } else {
                    if (matchesImplicitly(candidateDescriptor)) {
                        possibleValues.add(candidateDescriptor);
                    }
                }
            } else {
                if (!useExplicitFilter && matchesImplicitly(candidateDescriptor)) {
                    possibleValues.add(candidateDescriptor);
                }
            }
        }
